...
public class SVGImageView extends ImageView {
	...
	private LoadAssetTask loadAssetTask;
	...
	public void setImageAsset(String filename) {
		setImageAsset(filename, -1);
	}

	/**
	 * Load an SVG image from the given asset filename.
	 */
	public void setImageAsset(String filename, int color) {
		try {
			if (loadAssetTask != null) {
				loadAssetTask.cancel(true);
			}
			loadAssetTask = new LoadAssetTask();
			loadAssetTask.color = color;
			loadAssetTask.execute(filename);
		} catch (Throwable e) {
			e.printStackTrace();
		}
	}
	...
	private class LoadAssetTask extends AsyncTask<String, Integer, Picture> {
		...
		protected Picture doInBackground(String... filename) {
			try {
				...
				SVG svg = SVG.getFromAsset(getContext().getAssets(),
						filename[0]);
				if (color != -1) {
					svg.updateStyle(new ColorUpdaterInstanceFill(color));
				}
				return svg.renderToPicture();
			} catch (SVGParseException e) {
				...
				Log.e("SVGImageView", "Unable to load file: " + filename, e);
			}
			return null;
		}
		...
		protected void onPostExecute(Picture picture) {
			if (picture != null) {
				...
				onSvgLoaded(pictureDrawable, fileName);
	...
	public interface SvgLoadListener {
		public void onSvgLoaded(PictureDrawable svg, String string);
	}

	private SvgLoadListener svgLoadListener;

	public void setSvgLoadListener(SvgLoadListener svgLoadListener) {
		this.svgLoadListener = svgLoadListener;
	}

	public void onSvgLoaded(PictureDrawable svg, String string) {
		if (svgLoadListener != null) {
			svgLoadListener.onSvgLoaded(svg, string);
		}
