...
public class SVG {
	...
	public void updateStyle(StyleUpdater styleUpdater) {
		updateStyles(rootElement, styleUpdater);
		updateCssRules(styleUpdater);
		styleUpdater.updateStyle(style);
	}

	private void updateCssRules(StyleUpdater styleUpdater) {
		if (cssRules != null && cssRules.getRules() != null) {
			for (Rule rule : cssRules.getRules()) {
				if (rule != null) {
					styleUpdater.updateStyle(rule.style);
				}
			}
		}
	}

	private void updateStyles(SvgObject svgObject, StyleUpdater styleUpdater) {
		if (svgObject == null) {
			return;
		}
		if (svgObject instanceof SvgElementBase) {
			SvgElementBase object = ((SvgElementBase) svgObject);
			if (object != null) {
				styleUpdater.updateStyle(object.baseStyle);
				styleUpdater.updateStyle(object.style);
			}
		}
		if (svgObject instanceof SvgConditionalContainer) {
			SvgConditionalContainer object = (SvgConditionalContainer) svgObject;
			if (object != null) {
				for (SvgObject svg : object.children) {
					updateStyles(svg, styleUpdater);
				}
			}
		}
